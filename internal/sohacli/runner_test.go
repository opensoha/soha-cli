...
func TestCommandDocsFileIsFresh(t *testing.T) {
	...
	if err := writeCommandDocsMarkdown(&generated); err != nil {
...
func TestRunAddInteractiveSelectsTargets(t *testing.T) {
	...
	if !ok {
		t.Fatalf("cursor config has invalid mcpServers: %#v", decoded)
	}
...
func TestRunAddKiroUpdatesJSONCConfig(t *testing.T) {
	...
	servers, ok := decoded["mcpServers"].(map[string]any)
	if !ok {
		t.Fatalf("Kiro config has invalid mcpServers: %#v", decoded)
	}
	if _, ok := servers["old"]; !ok {
		t.Fatalf("existing MCP entry was not preserved: %#v", decoded)
	}
	soha, ok := servers["soha"].(map[string]any)
	if !ok {
		t.Fatalf("Kiro config has invalid soha entry: %#v", servers)
	}
