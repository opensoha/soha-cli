...
| `compute` | `soha compute <capabilities\|overview\|access-sources\|providers\|provider-instances\|resources\|tasks> [options]` | Inspect and operate the unified compute resource surface. | `soha compute overview`<br>`soha compute resources relations virtualization vm vm-1`<br>`soha compute tasks list --status failed` |
| `compute capabilities` | `soha compute capabilities [options]` | Show compute capability rollout and availability. | `soha compute capabilities --output yaml` |
| `compute overview` | `soha compute overview [options]` | Show the permission-filtered compute overview. | `soha compute overview` |
...
| `mcp` | `soha mcp [options] \| soha mcp <start\|install> [options]` | Run the Soha MCP stdio server or print client configuration. | `soha mcp`<br>`soha mcp --skill-id k8s-sre`<br>`soha mcp --base-url https://soha.example.com` |
...
| `setup` | `soha setup [target] [--client target] [--mode mcp\|skill\|both] [--scope user\|project] [options]` | Configure Soha MCP and skills for an AI agent or IDE. | `soha setup --client codex`<br>`soha setup --client codex --scope project`<br>`soha setup --client codex --mode both --skills k8s-sre --skill-id k8s-sre`<br>`soha setup --client claude --mode mcp --base-url https://soha.example.com`<br>`soha setup --client codex --check` |
